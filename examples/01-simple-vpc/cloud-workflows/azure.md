# Azure Workflow for Simple VPC

This guide shows the commands and prompts to deploy a simple Virtual Network on Azure using IaC Spec Kit.
...
```
/iac.principles I'm building basic networking infrastructure on Azure. Start simple, keep costs down, use Terraform.
```
...
Uses generic terms rather than cloud-specific services (e.g., "NAT gateway" not "Azure NAT Gateway"). Specific sections and detail vary by AI tool and LLM model.
...
```
/iac.plan Deploy in East US. Use Azure Bastion for secure access.
```

**What this does**: The AI researches Azure best practices, well-architected framework pillars, and Azure Verified Modules. It then designs the complete network architecture and generates planning documents.
...
- Networking: Virtual Network with address space, public/private subnets
- Connectivity: NAT Gateway for private subnet outbound access
- Security: Network Security Groups with least-privilege rules
- Access: Azure Bastion or VM bastion host
...
- Well-Architected Framework mappings (e.g., Security → network isolation, Cost Optimization → appropriate SKUs)
...
- Create Virtual Network with address space in iac/vnet.tf
- Configure public and private subnets in iac/subnets.tf
- Set up NAT Gateway in iac/nat.tf
- Configure route tables in iac/routes.tf
- Create Network Security Groups in iac/nsg.tf
...
- subnets.tf (public and private subnets), nat.tf (NAT Gateway)
- routes.tf (route tables), nsg.tf (Network Security Groups), bastion.tf
- outputs.tf
...
5. Access your Virtual Network using the output values
...
- **Principles are project-wide**: They apply to ALL features, not just this VNet.
